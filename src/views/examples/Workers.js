...
const App = () => {
  ...
  const [currentData, setCurrentData] = useState({Full_Name: '', Gender: '', Age: '', Certificate: '',Phone_Number:'',Status:'',Performance_Score:'',Has_Disability:'',Tool:'', Position: '', Remarks: '', Availability: '',Location:''
  });
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`${backendUrl}/Staff/workers/`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        response = await fetch(`${backendUrl}/Staff/workers/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
    ...
    <div dir="rtl">
      ...
      <div style={{ margin: '20px 20px' }}>
        ...
        >
          ...
          <Cell>{rowData => {
                const location = Locations.find(Location => Location.id === rowData.Location);
                return Location ? Location.Name : 'لا يوجد موقع';}}
          </Cell>
           </Column>
        ...
        <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)} size="md">
          ...
          <Modal.Body>
            ...
            <Form.Group controlId="tool">
                <Form.ControlLabel>الأدوات</Form.ControlLabel>
                <Form.Control name="Tool" value={currentData.Tool} onChange={(value) => setCurrentData({...currentData, Tool: value})} />
              </Form.Group>
            <Form.Group>
                <Checkbox
                  name="Availability"
            ...
            <Form.Group>
                  <Form.ControlLabel>الموقع</Form.ControlLabel>
                    <SelectPicker
                      ...
                      onChange={value => setCurrentData(prev => ({ ...prev, Location: value }))}
