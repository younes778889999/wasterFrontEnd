...
const App = () => {
  ...
  const [currentData, setCurrentData] = useState({Full_Name: '', Gender: '', Age: '',Phone_Number:'',Status:'',Performance_Score:'',Has_Disability:'', Position: '', Remarks: '', Availability: ''
  });
  ...
  const fetchPermissions = async () => {
    try {
      ...
      if (userPermissions && userPermissions.table_permissions["Drivers"]) {
        setPermissions({
          ...userPermissions.table_permissions["Drivers"],
          approval_chain: userPermissions.approval_chain["Drivers"] || {}
        });
      }
    } catch (error) {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${backendUrl}/Staff/drivers/`);
      ...
      setData(workersData); // Update your state with the data
      return workersData;
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        response = await fetch(`${backendUrl}/Staff/drivers/${currentData.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      } else {
        response = await fetch(`${backendUrl}/Staff/drivers/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      ...
      setCurrentData({ Full_Name: '', Gender: '', Age: '',Phone_Number:'',Status:'',Performance_Score:'',Has_Disability:'', Position: '', Remarks: '', Availability: ''});
      fetchData();
    } catch (error) {
    ...
    if (window.confirm('هل أنت متأكد أنك تريد حذف هذا الصف؟')) {
      try {
        ...
        if (needsApproval) {
          ...
          response = await fetch(`${backendUrl}/Staff/drivers/${id}`, {
            method: 'DELETE',
          });
    ...
    <div dir="rtl">
      ...
      {permissions.add && (
        ...
        >
          ...
          <Column width={75} align="center"  sortable>
            <HeaderCell>رقم الشاحنة</HeaderCell>
            <Cell dataKey="Truck"/>
          </Column>
          <Column width={50} align="center"  sortable>
          ...
          <Modal.Body>
            <Form fluid>
              ...
              <Form.Group controlId="performance_score">
                ...
                <Form.Control name="Performance_Score" type="number" value={currentData.Performance_Score} onChange={(value) => setCurrentData({...currentData, Performance_Score: parseInt(value)})} />
              </Form.Group>
              <Form.Group>
              <Checkbox
                name="Has_Disability"
